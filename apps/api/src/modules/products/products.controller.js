...
module.exports = {
  ...
  listCategories: async (r, res, n) => { try { const d = await s.listCategories(); res.json({ success: true, data: d }); } catch (e) { n(e); } },
  ...
  uploadImage: async (r, res, n) => {
    try {
      if (!r.file) throw new AppError('FILE_REQUIRED', 'An image file is required', 400);
      const url = publicUrl('products', r.file.filename);
      const d = await s.uploadImage(r.params.id, { url, originalname: r.file.originalname }, r.user.id);
  ...
  setBulkTiers: async (r, res, n) => { try { const d = await s.setBulkTiers(r.params.id, r.body.tiers, r.user.id); res.json({ success: true, data: d }); } catch (e) { n(e); } },
