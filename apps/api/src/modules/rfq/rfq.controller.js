...
module.exports = {
  ...
  list: async (r, res, n) => { try { const isAdmin = r.user.permissions.includes('ADMIN') || r.user.permissions.includes('SALES_MANAGER'); const d = await s.list(r.query, r.user.id, isAdmin); res.json({ success: true, ...d }); } catch (e) { n(e); } },
  get: async (r, res, n) => { try { const isAdmin = r.user.permissions.includes('ADMIN') || r.user.permissions.includes('SALES_MANAGER'); const d = await s.get(r.params.id, r.user.id, isAdmin); res.json({ success: true, data: d }); } catch (e) { n(e); } },
