    ...
    <div className={cn('space-y-4', className)}>
      {lines.map((line, idx) => (
        <div key={idx} className="grid gap-3 rounded-2xl bg-white/70 p-4 ring-1 ring-slate-100 md:grid-cols-[1fr_6rem_8rem_10rem_auto] md:items-end">
          ...
          <div><label className="mb-1 block text-xs font-semibold text-slate-500">Qty</label><input type="number" min={1} max={100000} step={1} value={line.quantity} onChange={(e) => updateLine(idx, 'quantity', Math.min(100000, Math.max(1, parseInt(e.target.value) || 1)))} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" /></div>
